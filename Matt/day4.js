...
// map input into array of arrays
...
data.forEach(row => {
  // sort array alphabetically to check for duplicates more easily
  const sortedRow = row.sort();
  let rowValid = true;

  for (i = 0; i < sortedRow.length; i++) {
    // check if word is the same as previous word
    if (sortedRow[i] === sortedRow[i - 1]) {
      // if it is, mark the row as invalid
      rowValid = false;
    }
  }

  // if the row was valid, increment valid passwords
  if (rowValid) {
    validPasswords += 1;
  }
});
