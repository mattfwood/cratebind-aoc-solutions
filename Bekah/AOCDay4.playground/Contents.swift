...
func validatePassphrase(passphrase: String) -> Int {
    var phraseCounts = [String]()
    var validPassPhrases = 0
    //split into array by new line
    let splitString = passphrase.components(separatedBy: "\n")
    //split into array of array by spaces
    for i in splitString {
        let splitAgainString = i.components(separatedBy: " ")
        // add each phrase to a new array, and if it already exists then delete a count from total
        for x in splitAgainString {
            if phraseCounts.contains(x) == false {
                phraseCounts.append(x)
                    print(phraseCounts)
            } else {
        ...
        //by default add one to count and then clear out array
        validPassPhrases = validPassPhrases + 1
        phraseCounts = []
    }
...
print(validatePassphrase(passphrase: passphrase))
